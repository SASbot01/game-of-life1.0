import { useEffect, useRef } from 'react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { supabase } from '@/integrations/supabase/client';
import { useAuth } from './useAuth';
import { toast } from 'sonner';
import { startOfDay, subDays, isAfter, isSameDay } from 'date-fns';

interface Habit {
  id: string;
  name: string;
  hp_penalty: number;
  last_completed_at: string | null;
  is_active: boolean;
  scheduled_time: string | null;
}

export function useHpDecay() {
  const { user, profile, refreshProfile } = useAuth();
  const queryClient = useQueryClient();
  const hasCheckedToday = useRef(false);

  // Fetch active habits
  const { data: habits } = useQuery({
    queryKey: ['habits-decay', user?.id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('habits')
        .select('id, name, hp_penalty, last_completed_at, is_active, scheduled_time')
        .eq('user_id', user?.id)
        .eq('is_active', true);

      if (error) throw error;
      return data as Habit[];
    },
    enabled: !!user?.id,
  });

  // No longer need game rules for HP decay - using individual habit penalties

  // Apply HP decay mutation
  const applyDecayMutation = useMutation({
    mutationFn: async ({ newHp, missedHabits }: { newHp: number; missedHabits: Habit[] }) => {
      const { error } = await supabase
        .from('profiles')
        .update({ hp: Math.max(0, newHp) })
        .eq('id', user?.id);

      if (error) throw error;
      return { newHp, missedHabits };
    },
    onSuccess: ({ newHp, missedHabits }) => {
      refreshProfile();
      queryClient.invalidateQueries({ queryKey: ['profiles'] });

      // Show individual notifications for each missed habit
      missedHabits.forEach(habit => {
        toast.error(`❤️ -${habit.hp_penalty || 10} HP: ${habit.name}`, {
          description: 'Habit not completed yesterday',
          duration: 4000,
        });
      });
    },
  });

  useEffect(() => {
    if (!user || !profile || !habits || hasCheckedToday.current) return;

    // Check localStorage for last decay check
    const lastDecayCheck = localStorage.getItem(`hp_decay_check_${user.id}`);
    const today = startOfDay(new Date());

    if (lastDecayCheck && isSameDay(new Date(lastDecayCheck), today)) {
      hasCheckedToday.current = true;
      return; // Already checked today
    }

    // Calculate missed habits from yesterday
    const yesterday = subDays(today, 1);
    const missedHabits = habits.filter(habit => {
      if (!habit.last_completed_at) {
        // Never completed - only penalize if habit existed before yesterday
        return true;
      }

      const lastCompleted = new Date(habit.last_completed_at);
      // If last completed before yesterday, it's missed
      return !isAfter(lastCompleted, yesterday) && !isSameDay(lastCompleted, yesterday);
    });

    if (missedHabits.length > 0 && habits.length > 0) {
      // Calculate total penalty using individual habit penalties
      const totalPenalty = missedHabits.reduce((sum, habit) => sum + (habit.hp_penalty || 10), 0);
      const newHp = Math.max(0, profile.hp - totalPenalty);

      // Apply decay
      applyDecayMutation.mutate({ newHp, missedHabits });
    }

    // Mark as checked for today
    localStorage.setItem(`hp_decay_check_${user.id}`, today.toISOString());
    hasCheckedToday.current = true;

  }, [user, profile, habits, applyDecayMutation]);

  return null;
}
