...
export function BioActionTab() {
  ...
  const [newHabit, setNewHabit] = useState({
    ...
    xp_reward: 10,
    hp_penalty: 10, // Add HP penalty field
    area_id: "",
  ...
  const createHabitMutation = useMutation({
    mutationFn: async (habit: typeof newHabit) => {
      const { error } = await supabase.from("habits").insert({
        ...
        hp_impact: 5,
        hp_penalty: habit.hp_penalty, // Add HP penalty
    ...
    onSuccess: () => {
      ...
      setDialogOpen(false);
      setNewHabit({ name: "", category: "health", xp_reward: 10, hp_penalty: 10, area_id: "", scheduled_time: "" });
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="bg-card border-bio/30">
            ...
            <div className="space-y-4 pt-4">
              ...
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">HP Penalty (if not completed)</label>
                <Select
                  value={newHabit.hp_penalty.toString()}
                  onValueChange={(v) => setNewHabit({ ...newHabit, hp_penalty: parseInt(v) })}
                >
                  <SelectTrigger className="bg-background border-border">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="5">5 HP (Low)</SelectItem>
                    <SelectItem value="10">10 HP (Normal)</SelectItem>
                    <SelectItem value="15">15 HP (Important)</SelectItem>
                    <SelectItem value="20">20 HP (Critical)</SelectItem>
                    <SelectItem value="30">30 HP (Essential)</SelectItem>
                  </SelectContent>
                </Select>
                <p className="text-xs text-muted-foreground mt-1">HP lost if habit is not completed daily</p>
              </div>
              <div>
